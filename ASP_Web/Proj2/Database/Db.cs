...
namespace Proj2.Database {
    public class DatabaseContext : DbContext {
        ...
        public DbJsonLog[] GetJsonLog(DbDeviceAPIKey APIKey, int Count = 0) {
            if (APIKey == null)
                return null;

            IQueryable<DbJsonLog> QueryResult = JsonLog.AsNoTracking().Where(JsonLog => JsonLog.CreatedByKey.ID == APIKey.ID).OrderByDescending(JsonLog => JsonLog.ID);

            if (Count > 0) {
                QueryResult = QueryResult.Take(Count);
            }

            DbJsonLog[] Result = QueryResult.ToArray();
            return Result;
        }
    }
}
